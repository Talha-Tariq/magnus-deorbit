...
% Thruster Effects
if (AuxParam.Thruster)
%     if (mod(t,5918.4)<(5918.4/2)) %validation thruster
% %         pos = Y(1:3,:); %m
%         vel = Y(4:6,:); %m/s
%         vel_norm = vel/norm(vel);
%         a_thrust = 0.15/AuxParam.mass;
%         a_vec = a_thrust*vel_norm;
%         a = a + a_vec;
%     end
    pos = Y(1:3,:); %m
    alt = norm(pos)/1000 - 6378.137;
    Cl = (1/3) - (5/3)*tanh(2*alt - 164);
