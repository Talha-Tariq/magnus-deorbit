...
constants
load DE436Coeff.mat
PC = DE436Coeff; %ephemeris of other planets?

% read Earth gravity field coefficients
Cnm = zeros(181,181);
Snm = zeros(181,181);
fid = fopen('GGM03S.txt','r'); %gravity model between 2003 and 2006
for n=0:180
    for m=0:n
...
% epoch state (Envisat)
fid = fopen('InitialState2.txt','r'); %ECEF frame used in this file (for IS1)
tline = fgetl(fid);
...
% epoch
Mjd_UTC = Mjday(year, mon, day, hour, min, sec); %Modified Julian Date from calendar date and time
% Y0 = ECEF2ECI(Mjd_UTC, Y0); %Initial pos and vel in ECI Frame
...
% propagation
Eph = Ephemeris(Y0, N_Step, Step); %Y0 in ECI, outputs in ECI as well

fid = fopen('SatelliteStates.txt','w');
for i=1:N_Step+1 %write satellite states
...
[n, m] = size(Eph);
Eph_ecef = zeros(n,m);
for i=1:n %create ephemeris in ECEF
    Eph_ecef(i,1) = Eph(i,1);
    Eph_ecef(i,2:7) = ECI2ECEF(Mjd0+Eph_ecef(i,1)/86400, Eph(i,2:7));    
end

% True_EnvisatStates %True Ephemeris of Envisat in ECEF
% dd = True_Eph-Eph_ecef(:,2:7); %error between true and simulated

%norm of position vectors
for i = 1:length(Eph)
pos_norm(i) = norm(Eph(i,2:4));
end
pos_norm = pos_norm';
